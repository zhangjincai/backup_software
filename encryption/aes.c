 ...
 */
static void aes_set_key( aes_context *ctx, uchar *key, uint keysize )
{
    uint i;
    ulong *RK, *SK;
 ...
 */
static void aes_encrypt( aes_context *ctx, uchar input[16], uchar output[16] )
{
 ...
 */
static void aes_decrypt( aes_context *ctx, uchar input[16], uchar output[16] )
{
    ulong *RK, X0, X1, X2, X3, Y0, Y1, Y2, Y3;
 ...
 */
#if 0
static void aes_cbc_encrypt( aes_context *ctx, uchar iv[16],
 ...
 */
#if 0
static void aes_cbc_decrypt( aes_context *ctx, uchar iv[16],
                      uchar *input, uchar *output, uint len )
{
    int i;
    uchar temp[16];
...
void lib_aes_encrypt(unsigned char *out, unsigned char *in, const int inlen, unsigned char *key, const unsigned int keysize)
{
	...
	{
		aes_encrypt(&ctx, in+i*16, out+i*16);
	}
...
void lib_aes_decrypt(unsigned char *out, unsigned char *in, const int inlen, unsigned char *key, const unsigned int keysize)
{
	...
	for(i = 0; i < (inlen / 16); i++)
	{
		aes_decrypt(&ctx, in+i*16, out+i*16);
	}
