...
static void des_main_ks( ulong SK[32], uchar key[8] )
{
    int i;
    ulong X, Y, T;
 ...
 * DES key schedule
 */
static void des_set_key( des_context *ctx, uchar key[8] )
{
    int i;
...
static void des_crypt( ulong SK[32], uchar input[8], uchar output[8] )
{
    ulong X, Y, T;
 ...
 */
static void des_encrypt( des_context *ctx, uchar input[8], uchar output[8] )
{
 ...
 */
 #if 0
static void des_cbc_encrypt( des_context *ctx, uchar iv[8],
 ...
 * DES-CBC decryption
 */
static void des_cbc_decrypt( des_context *ctx, uchar iv[8],
                      uchar *input, uchar *output, uint len )
{
    int i;
    uchar temp[8];
 ...
 */
static void des3_set_2keys( des3_context *ctx, uchar key[16] )
{
    int i;
 ...
 */
 #if 0
static void des3_set_3keys( des3_context *ctx, uchar key[24] )
{
    int i;
...
static void des3_crypt( ulong SK[96], uchar input[8], uchar output[8] )
{
    ulong X, Y, T;
 ...
 */
static void des3_encrypt( des3_context *ctx, uchar input[8], uchar output[8] )
{
 ...
 */
 #if 0
static void des3_cbc_encrypt( des3_context *ctx, uchar iv[8],
                       uchar *input, uchar *output, uint len )
{
    int i;
 ...
 * 3DES-CBC decryption
 */
static void des3_cbc_decrypt( des3_context *ctx, uchar iv[8],
                       uchar *input, uchar *output, uint len )
{
    int i;
    uchar temp[8];
...
void lib_des3_encrypt(unsigned char *out, unsigned char *in, const int inlen, unsigned char *key)
{
	...
	for(i = 0; i < (inlen / 8); i++)
	{
		des3_encrypt(&ctx, in+i*8, out+i*8);
	}
	...
	int i;
	unsigned char m_out[BUF_MAX_SIZE] = {0};
	unsigned char m_in[BUF_MAX_SIZE] = {0};
	
	memcpy(m_in, in, inlen);
	
	for(i = 0; i < inlen;)
	{
		lib_des3(&m_out[i], &m_in[i], key, 0);
		i += 8;
	}

	memcpy(out, m_out, inlen);
}
...
void lib_des3_decrypt(unsigned char *out, unsigned char *in, const int inlen, unsigned char *key)
{
	...
	for(i = 0; i < (inlen / 8); i++)
	{
		des3_decrypt(&ctx, in+i*8, out+i*8);
	}
