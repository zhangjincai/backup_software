...
void build_request(const char *url)
{
  ...
  if(proxyhost==NULL)
	   if (0!=strncasecmp("http://",url,7)) //URL必须以http://开头
	   { 
  ...
  i=strstr(url,"://")-url+3;//stlen("http://")//i指向http://后第一个位置(相对位置)
  
  //printf("strlen:%d\n",strlen("http://"));
  //printf("--------i:%d---------\n",i);

  if(strchr(url+i,'/')==NULL) //URL必须以'/'结尾
  {
  ...
  if(proxyhost==NULL)
  {
   ...
   {
	   strncpy(host,url+i,strchr(url+i,':')-url-i);//ip(域名)
	   bzero(tmp,10);
	   strncpy(tmp,index(url+i,':')+1,strchr(url+i,'/')-index(url+i,':')-1);//端口
	   /* printf("tmp=%s\n",tmp); */
	   proxyport=atoi(tmp);
	   if(proxyport==0) proxyport=80;
   } else
   {
     strncpy(host,url+i,strcspn(url+i,"/"));//strcspn:返回字符串url+i开头连续不含字符串"/"内的字符数目.
   }
   //printf("Host=%s\n",host);
...
static int bench(void)
{
  ...
  i=Socket(proxyhost==NULL?host:proxyhost,proxyport);//检查合法性、socket、connect
  if(i<0) { 
  ...
  */
  ...
  if(pid== (pid_t) 0)//如果是子进程，调用benchcore进行测试
  {
    ...
    if(proxyhost==NULL)
	 ...
	 f=fdopen(mypipe[1],"w");//子进程将测试结果输出到管道 一个管道只能进行半双工的工作(一端读，一端写)
	 if(f==NULL)
	 {
  ...
  }else//如果是父进程，则从管道读取子进程输出，并作汇总
  {
	  ...
	  setvbuf(f,NULL,_IONBF,0);//_IONBF:无缓冲
	  
	  speed=0;/* 虽然子进程不会污染父进程的这几个变量，但用前重置一下，在这里是个好习惯 */
      failed=0;
      bytes=0;
	  ...
	  while(1)
	  {
		  ...
		  speed+=i;//speed表示成功数，failed表示失败数
		  failed+=j;
		  bytes+=k;//记录进程成功读取的字节数
...
void benchcore(const char *host,const int port,const char *req)
{
 ...
 nexttry:
 while(1)/* 无限执行请求，直到接收到 SIGALRM 信号将 timerexpired 设置为 1 时 */
 {
    if(timerexpired)//闹钟过时
    {
       if(failed>0)
       {
    ...
    if(http10==0) 
	    if(shutdown(s,1)) { failed++;close(s);continue;}//关掉写端
    if(force==0) /* 如果等待响应数据返回，则读取响应数据，计算传输的字节数 */
    {
    ...
    speed++;/* 成功完成一次请求，并计数，继续下一次相同的请求，直到超时为止 */
 }
}
