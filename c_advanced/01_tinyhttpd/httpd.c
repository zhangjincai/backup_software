...
void accept_request(int client)
{
 ...
 numchars = get_line(client, buf, sizeof(buf));// 获取请求报文的第一行，即为请求行
 i = 0; j = 0;
 ...
 if (strcasecmp(method, "POST") == 0)//向指定的资源提交要被处理的数据
  cgi = 1;
 // 获取url
 i = 0;
 ...
 if (strcasecmp(method, "GET") == 0)//从指定的资源请求数据
 {
  query_string = url;
...
int startup(u_short *port)
{
 ...
 httpd = socket(PF_INET, SOCK_STREAM, 0);//ipv4
 if (httpd == -1)
  error_die("socket");
...
int main(void)
{
 ...
 server_sock = startup(&port);//创建一个用于监听的服务端socket并指定监听端口
