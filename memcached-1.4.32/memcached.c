...
conn *conn_new(const int sfd, enum conn_states init_state,
	...
	//conn_new函数对套接字设置conn_listening监听事件，回调函数为event_handler，在事件响应函数中调用状态机。
...
//当监听套接字建立新连接时，通过事件响应函数event_handler来触发状态机，再调用dispatch_conn_new调度新线程来处理这个连接的读写事件。
static void drive_machine(conn *c) {
    bool stop = false;
    int sfd;
    socklen_t addrlen;
    ...
    while (!stop) {
        ...
        case conn_listening:
            ...
            if (!use_accept4) {
                if (fcntl(sfd, F_SETFL, fcntl(sfd, F_GETFL) | O_NONBLOCK) < 0) { //新套接字设置非阻塞
            ...
            if (settings.maxconns_fast &&
                ...
                dispatch_conn_new(sfd, conn_new_cmd, EV_READ | EV_PERSIST,
                                     DATA_BUFFER_SIZE, c->transport);  //调度线程来处理连接
            }
...
static int server_socket_unix(const char *path, int access_mask) {
    ...
    setsockopt(sfd, SOL_SOCKET, SO_LINGER, (void *)&ling, sizeof(ling)); //如果有数据残留在套接口缓冲区中则系统将试着将这些数据发送给对方
...
static int enable_large_pages(void) {
    ...
    int ret = -1;
    size_t sizes[32];
    int avail = getpagesizes(sizes, 32); //获取内存分页大小
    if (avail != -1) {
        size_t max = sizes[0];
...
int main (int argc, char **argv) {
    ...
    /* init settings */
    settings_init(); //初始化全局变量settings函数
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        case 'a':
            ...
            settings.access= strtol(optarg,NULL,8); //strtol:string to int
            break;

        case 'U':
            settings.udpport = atoi(optarg); //atoi:strtol(nptr, (char **)NULL, 10);
            udp_specified = true;
            break;
        case 'p':
        ...
        case 'l':
            if (settings.inter != NULL) {
                ...
                settings.inter= strdup(optarg); //copy string
            }
            break;
        case 'd':
            do_daemonize = true;
        ...
        case 'P':
            ...
            break;
        case 'f':
            settings.factor = atof(optarg); //string to a double
    ...
    if (lock_memory) {
#ifdef HAVE_MLOCKALL
        int res = mlockall(MCL_CURRENT | MCL_FUTURE); //lock memory
        if (res != 0) {
	...
	//创建监听套接字，memcached支持TCP，UDP，UNIX域套接字，因此相应的要创建三种监听套接字
    ...
    if (settings.socketpath == NULL) { //TCP/UDP
