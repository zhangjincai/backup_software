...
static void wait_for_thread_registration(int nthreads) {
    while (init_count < nthreads) {
        pthread_cond_wait(&init_cond, &init_lock); //即主线程阻塞如此，等待worker_libevent发出的
        //init_cond信号，唤醒后检查init_count < nthreads是否为假（即创建的线程数目是否达到要求），否则继续等待。
    }
}
...
static void setup_thread(LIBEVENT_THREAD *me) {
    me->base = event_init(); //初始化struct event_base对象
    if (! me->base) {
    ...
    event_set(&me->notify_event, me->notify_receive_fd,
              EV_READ | EV_PERSIST, thread_libevent_process, me); //初始化struct event对象
    event_base_set(me->base, &me->notify_event); //调用event_base_set重新指定event_base对象
...
static void *worker_libevent(void *arg) {
	...
	//新创建线程阻塞于此，等待事件
    event_base_loop(me->base, 0); //事件循环，事件发生后，调用相应回调函数
    return NULL;
}
...
static void thread_libevent_process(int fd, short which, void *arg) {
    ...
    switch (buf[0]) {
    case 'c':
        item = cq_pop(me->new_conn_queue); //从链接队列中取出一个conn
...
void dispatch_conn_new(int sfd, enum conn_states init_state, int event_flags,
    ...
    LIBEVENT_THREAD *thread = threads + tid; //以此种方式来取出线程
    ...
    cq_push(thread->new_conn_queue, item); //将新item放至threads的new_conn_queue队列中

    MEMCACHED_CONN_DISPATCH(sfd, thread->thread_id);
    buf[0] = 'c';
    if (write(thread->notify_send_fd, buf, 1) != 1) { //写一个字节启动新的线程
...
void memcached_thread_init(int nthreads) {
    ...
    threads = calloc(nthreads, sizeof(LIBEVENT_THREAD));//分配线程池结构数组
    if (! threads) {
        perror("Can't allocate thread descriptors");
        exit(1);
    }
	
	//为线程池每个线程创建读写管道,该pipe将在线程的调度中发挥作用
    for (i = 0; i < nthreads; i++) {
		...
		//填充线程结构体信息,为线程设置事件监听，绑定CQ链表等初始化信息
        setup_thread(&threads[i]);
        /* Reserve three fds for the libevent base, and two for the pipe */
        stats_state.reserved_fds += 5;
    }

	//为线程池创建数目为nthreads的线程
