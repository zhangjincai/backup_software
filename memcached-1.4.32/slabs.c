...
void slabs_init(const size_t limit, const double factor, const bool prealloc, const uint32_t *slab_sizes) {
    ...
    if (prealloc) {
        ...
        if (mem_base != NULL) {
            mem_current = mem_base; //mem_current:静态变量，记录分配内存块的基地址
            mem_avail = mem_limit; //mem_avail:可用内存大小
        } else {
    ...
    while (++i < MAX_NUMBER_OF_SLAB_CLASSES-1) {
        ...
        slabclass[i].perslab = settings.slab_page_size / slabclass[i].size; //记录每个slab中item的个数
        if (slab_sizes == NULL)
...
static int grow_slab_list (const unsigned int id) {
    ...
    if (p->slabs == p->list_size) { //表示slabs已经用完 p->slabs:已经分配的slab数，p->list_size:slab链表的长度
...
/*
这个函数的作用是当一个slab用光后，又有新的item要插入这个id，那么它就会重新申请新的slab，\
申请新的slab时，对应id的slab链表就要增长（由grow_slab_list（）函数来实现），这个链表是成倍增长的，初始化值为16。
*/
static int do_slabs_newslab(const unsigned int id) {
    slabclass_t *p = &slabclass[id];
