...
static item *heads[LARGEST_ID]; //保存各个slab class所对应的item链表的表头。
static item *tails[LARGEST_ID]; //保存各个slab class所对应的item链表的表尾。
static itemstats_t itemstats[LARGEST_ID];
static unsigned int sizes[LARGEST_ID]; //保存各个slab class所对应的items数目。
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv, conn *c) {
    ...
    if (it != NULL) {
        ...
        if (item_is_flushed(it)) {
            ...
            do_item_unlink(it, hv); //该item已过期，则回收该item，，调用do_item_unlink()把它从链表中取出来。
            do_item_remove(it);
            it = NULL;
            pthread_mutex_lock(&c->thread->stats.mutex);
